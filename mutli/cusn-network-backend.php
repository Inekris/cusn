...
class ContentUnwantedScraperNetwork
{
    ...
    public function __construct()
    {
        ...
        add_action( 'network_admin_edit_cusn_set', array( $this, 'cusn_admin_sanitize' ) );
        add_action( 'admin_init', array( $this, 'page_init' ) );
        $this->htaccess_options = get_site_option( 'cusn_site_htaccess_options' );
    ...
    public function create_admin_page()
    {
		if ( isset( $_GET['updated']) ) {
			...
			<?php }
				...
				<div id="message" class="error">
					<p><strong><?php _e('Settings not saved.'); 
						if ( isset( $_GET['error-text'] ) ) echo $_GET['error-text'];
					?></strong></p>
				</div>
			<?php	
			}
        ...
        ?>            
	         	<div>
		            <form method="post" action="edit.php?action=cusn_set">
		            	<?php
		            	wp_nonce_field('cusn_set', 'cusn_nonce');
...
*
*/
	public function cusn_admin_sanitize() {
		if ( ! wp_verify_nonce( $_REQUEST['cusn_nonce'], 'cusn_set' ) ) {
			wp_redirect( ( add_query_arg( array( 'page' => 'cusn-settings', 'updated' => 'false' ), network_admin_url('settings.php') ) ) );
			exit();
			}

		$new_input = $this->cusn_htaccess_sanitize( $_POST['cusn-htaccess-settings'] );
		update_site_option('cusn_site_htaccess_options', $new_input );
		wp_redirect( ( add_query_arg( array( 'page' => 'cusn-settings', 'updated' => 'true' ), network_admin_url('settings.php') ) ) );
		exit();
	}
